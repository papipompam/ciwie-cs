...
export interface DocumentBatchRepository {
  ...
  findMemberForUpdate(id: string): Promise<{ id: string, batchId: string, requestId: string } | null>
...
export async function removeDocumentBatchMember(
  repository: DocumentBatchRepository,
  actor: SessionActor,
  input: { batchId: string, memberId: string, expectedBatchVersion: number, reason: string },
): Promise<{ removed: true }> {
  requireRole(actor, 'LECTURER', 'ADMIN')
  return await repository.transaction(async (tx) => {
    const [batch, member] = await Promise.all([tx.findBatchForUpdate(input.batchId), tx.findMemberForUpdate(input.memberId)])
    if (!batch || !member || member.batchId !== input.batchId) throw new DomainError('NOT_FOUND', 'Batch member was not found')
    if (actor.role === 'LECTURER' && !batch.activeTerm) throw new DomainError('FORBIDDEN', 'Lecturers can edit batches only in the active term')
    if (batch.status !== 'DRAFT') throw new DomainError('INVALID_STATE', 'Members can be removed only from a draft batch')
    if (batch.version !== input.expectedBatchVersion) throw new DomainError('CONFLICT', 'Batch changed; reload and try again')
    await tx.removeMember(member.id)
    const changed = await tx.incrementBatchVersion(batch.id, input.expectedBatchVersion, actor.userId)
    if (!changed) throw new DomainError('CONFLICT', 'Batch changed; reload and try again')
    await tx.appendAudit({ actorId: actor.userId, action: 'DOCUMENT_BATCH_MEMBER_REMOVED', entityId: batch.id, before: { ...batch, member }, after: { removedMemberId: member.id, reason: input.reason } })
    return { removed: true }
  })
}
