...
import { updateResponseResults } from '../../../services/response-placement-service'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const body = parseStrict(responseDraftResultsSchema, await readBody(event))
    return await updateResponseResults(getWorkflowRepositories(event).responses, actor, { responseId, ...body })
  } catch (error) {
