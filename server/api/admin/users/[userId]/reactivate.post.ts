...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    return await runIdempotent({ actorId: actor.userId, operation: 'USER_REACTIVATE', key, request: { userId, ...body }, work: () => changeUserStatus(prisma, actor, userId, 'REACTIVATE', body.reason) })
