...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const idempotency = { actorId: actor.userId, operation: 'VISIT_CANCEL', key }
    return await runIdempotent({ ...idempotency, request: { visitId, ...body }, work: () => changeVisitSchedule(prisma, actor, { visitId, action: 'CANCEL', ...body, idempotency }) })
