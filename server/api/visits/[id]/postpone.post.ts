...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const idempotency = { actorId: actor.userId, operation: 'VISIT_POSTPONE', key }
    return await runIdempotent({ ...idempotency, request: { visitId, ...body }, work: () => changeVisitSchedule(prisma, actor, { visitId, action: 'POSTPONE', ...body, idempotency }) })
