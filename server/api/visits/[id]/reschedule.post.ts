...
import { visitRescheduleCommandSchema } from '../../../../shared/schemas/commands'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const body = parseStrict(visitRescheduleCommandSchema, await readBody(event))
    const idempotency = { actorId: actor.userId, operation: 'VISIT_RESCHEDULE', key }
    return await runIdempotent({ ...idempotency, request: { visitId, ...body }, work: () => changeVisitSchedule(prisma, actor, { visitId, action: 'RESCHEDULE', ...body, idempotency }) })
