...
export default defineEventHandler(async (event) => {
  ...
  const logs = user.role === 'staff' && people.length ? await prisma.auditLog.findMany({
    where: { entityType: 'User', entityId: { in: people.map(person => person.id) } },
    select: personAuditSelect, orderBy: { occurredAt: 'desc' }, take: Math.min(people.length * 100, 50_000),
  }) : []
