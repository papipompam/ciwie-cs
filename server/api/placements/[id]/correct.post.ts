...
import { placementCorrectSchema } from '../../../../shared/schemas/commands'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const body = parseStrict(placementCorrectSchema, await readBody(event))
    const idempotency = { actorId: actor.userId, operation: 'PLACEMENT_CORRECT', key }
    return await runIdempotent({ ...idempotency, request: { placementId, ...body }, work: () => changePlacement(prisma, actor, { placementId, action: 'CORRECT', ...body, idempotency }) })
