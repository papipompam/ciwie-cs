...
test('student sees only student navigation and is blocked from admin expense', async ({ page }) => {
  ...
  await expect(navigation.getByRole('link', { name: 'ข้อมูลการยื่นฝึก' })).toBeVisible()
  await expect(navigation.getByRole('link', { name: 'ข้อมูลนักศึกษา' })).toHaveCount(0)
...
test('lecturer can open evaluation work queue', async ({ page }) => {
  ...
  await (await openNavigation(page)).getByRole('link', { name: 'การประเมินผล' }).click()
...
test('admin list supports three-state server sort and accessible table controls', async ({ page }) => {
  ...
  await (await openNavigation(page)).getByRole('link', { name: 'ข้อมูลการยื่นฝึก' }).click()
...
test('student records every shared-response member result before submitting review', async ({ page }) => {
  ...
  await loginAs(page, 'STUDENT')
  await (await openNavigation(page)).getByRole('link', { name: 'แบบตอบรับเอกสารขออนุญาต' }).click()
...
test('admin previews selected import rows before atomic confirmation', async ({ page }) => {
  ...
  await loginAs(page, 'ADMIN')
  await (await openNavigation(page)).getByRole('link', { name: 'ข้อมูลนักศึกษา' }).click()
...
test('admin student-roster export uses uppercase format, report kind and coop term', async ({ page }) => {
  ...
  await page.getByLabel('ปีการศึกษา / ภาคเรียน / รอบสหกิจ', { exact: true }).selectOption('term-1')
  await page.getByRole('button', { name: 'สร้างไฟล์' }).click()
