...
import { ArrowDown, ArrowUp, ChevronLeft, ChevronRight, RotateCcw, Search, X } from '@lucide/vue'
  ...
  <div class="space-y-6">
      ...
      <template v-else>
        <div class="hidden overflow-x-auto md:block">
          <table class="w-full min-w-[1050px] text-left text-sm">
            ...
            <thead class="bg-surface text-xs text-muted"><tr><th scope="col" class="w-44 px-6 py-3">หมายเลขคำร้อง</th><th scope="col" class="px-4 py-3">นักศึกษา</th><th scope="col" class="px-4 py-3">บริษัท / ตำแหน่ง</th><th scope="col" class="px-4 py-3">เรียน (ผู้รับหนังสือ)</th><th scope="col" class="px-4 py-3" :aria-sort="ascending ? 'ascending' : 'descending'"><button class="inline-flex min-h-9 items-center gap-1" type="button" @click="ascending = !ascending">วันที่ส่งคำร้อง<ArrowUp v-if="ascending" :size="15" /><ArrowDown v-else :size="15" /></button></th><th scope="col" class="px-4 py-3">ขั้นตอนปัจจุบัน</th><th scope="col" class="px-4 py-3">จัดการ</th></tr></thead>
            <tbody class="divide-y divide-divider"><tr v-for="item in rows" :key="item.id" class="hover:bg-surface/70"><td class="px-6 py-4"><code class="inline-flex rounded bg-surface px-2 py-1 text-xs font-semibold tracking-wide text-ink">{{ item.requestNo ?? item.id }}</code></td><td class="px-4 py-4"><p class="font-semibold text-ink">{{ item.studentName }}</p><p class="mt-1 text-xs text-muted">{{ item.application.studentId }}</p></td><td class="max-w-xs break-words px-4 py-4"><p class="font-semibold text-ink">{{ item.application.companyName }}</p><p class="mt-1 text-xs text-muted">{{ item.application.position }}</p></td><td class="max-w-xs break-words px-4 py-4">{{ item.application.recipientName || 'ยังไม่ระบุ' }}</td><td class="px-4 py-4 text-muted">{{ date(item.submittedAt) }}</td><td class="px-4 py-4"><UiBadge :tone="requestStatusMeta[item.status].tone">{{ requestStatusMeta[item.status].label }}</UiBadge></td><td class="px-4 py-4"><UiButton variant="ghost" :aria-label="`แก้ไขคำร้อง ${item.studentName}`" title="แก้ไขคำร้อง" @click="open(item.id)">แก้ไข</UiButton></td></tr></tbody>
          </table>
        </div>
        <div class="divide-y divide-divider md:hidden"><article v-for="item in rows" :key="item.id" class="space-y-3 p-5"><div class="flex items-start justify-between gap-3"><div class="min-w-0 break-words"><code class="inline-flex rounded bg-surface px-2 py-1 text-xs font-semibold tracking-wide text-ink">{{ item.requestNo ?? item.id }}</code><h4 class="mt-2 font-semibold text-ink">{{ item.studentName }}</h4><p class="text-xs text-muted">{{ item.application.studentId }}</p></div><UiButton variant="ghost" :aria-label="`แก้ไขคำร้อง ${item.studentName}`" title="แก้ไขคำร้อง" @click="open(item.id)">แก้ไข</UiButton></div><p class="break-words text-sm">{{ item.application.companyName }} · {{ item.application.position }}</p><p class="break-words text-sm text-muted">เรียน: {{ item.application.recipientName }}</p><p class="text-xs text-muted">ส่งคำร้อง {{ date(item.submittedAt) }}</p><UiBadge :tone="requestStatusMeta[item.status].tone">{{ requestStatusMeta[item.status].label }}</UiBadge></article></div>
    ...
    <UiDialog v-model:open="detailOpen" size="lg" title="ดำเนินการคำร้อง" :description="selected ? `${selected.studentName} · ${selected.application.studentId}` : undefined">
      <div v-if="selected" class="space-y-5">
        ...
        <div class="border-t border-divider pt-5"><AppRequestDocuments :key="selected.id" :request="selected" staff @refresh="refreshRequests" /></div>
      </div>
    </UiDialog>
  </div>
</template>
