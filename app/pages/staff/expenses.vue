...
const { cycleId, cycleOptions, round, roundModel } = useSupervisionContext()
...
const { status, error, refresh } = await useAsyncData(
  ...
  // รอบสหกิจโหลดแบบ asynchronous จึงอาจยังไม่มี cycleId ในช่วง SSR/เริ่มต้น
  // ป้องกันการส่ง query ว่างไปยัง API ซึ่งจะทำให้เกิด 400 Bad Request
  () => cycleId.value ? loadExpenses(cycleId.value, round.value) : Promise.resolve({ groups: [], records: [] }),
...
<template>
  <div class="space-y-6">
    ...
    <!-- เจ้าหน้าที่เลือกปีการศึกษาและครั้งที่นิเทศได้ แม้ยังไม่มีกลุ่มนิเทศ -->
    <div class="flex flex-wrap justify-end gap-3">
      <div class="w-full sm:w-52"><UiSelect v-model="cycleId" :options="cycleOptions" label="ปีการศึกษา / ภาคเรียน" /></div>
      <div class="w-full sm:w-40"><UiInput v-model="roundModel" type="number" :min="1" :max="99" :step="1" label="ครั้งที่นิเทศ" /></div>
    </div>
