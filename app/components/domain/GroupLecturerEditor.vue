...
const { groups, supervisionLecturers, persistLecturers } = useSupervisionGroups()
...
const candidates = computed(() => {
  ...
  return supervisionLecturers.value.filter(person => !assigned.has(person.id))
})
...
const save = async () => {
  ...
  busy.value = true
  try { await persistLecturers(props.group.id, selected.value); showToast({ title: 'บันทึกอาจารย์ในกลุ่มแล้ว' }); error.value = '' }
...
<template>
  <form class="mt-5 space-y-3 rounded-control border border-divider p-4" @submit.prevent="save">
    ...
    <div v-for="person in candidates" :key="person.id" class="flex items-center gap-3"><UiCheckbox :model-value="selected.includes(person.id)" :label="person.name" @update:model-value="toggle(person.id, $event)" /><span class="text-sm text-ink">{{ person.name }}</span></div>
