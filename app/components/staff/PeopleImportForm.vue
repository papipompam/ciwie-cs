...
const handleImport = async () => {
  ...
  try {
    const imported = await persistImportPeople(props.personType, importableRows.value.map(row => ({ id: row.id, prefix: row.prefix, firstName: row.firstName, lastName: row.lastName, ...(row.phone ? { phone: row.phone } : {}), ...(row.email ? { email: row.email } : {}), ...(props.personType === 'student' && selectedCycle.value ? { cohortYear: selectedCycle.value.targetCohortYear, cycle: selectedCycle.value.label } : {}), ...(row.section ? { section: row.section } : {}) })))
...
<template>
  <div>
    <template v-if="stage === 'upload'">
      ...
      <UiSelect v-if="props.personType === 'student'" v-model="selectedCycleId" class="mt-5 w-full sm:max-w-md" :options="cycleOptions" label="รอบสหกิจศึกษา" placeholder="เลือกรอบสหกิจศึกษา" />
    ...
    <template v-else-if="stage === 'preview'">
      ...
      <UiAlert v-if="props.personType === 'student'" class="mt-4" tone="info" title="รอบสหกิจศึกษาที่เลือก">นักศึกษาชุดนี้จะถูกบันทึกใน {{ selectedCycle?.label }}</UiAlert>
    ...
    <template v-else>
      ...
      <UiAlert v-if="props.personType === 'student'" class="mt-4" tone="info" title="รอบสหกิจศึกษาที่นำเข้า">บันทึกข้อมูลนักศึกษาใน {{ selectedCycle?.label }}</UiAlert>
