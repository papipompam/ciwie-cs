...
const handleImport = async () => {
  ...
  try {
    const imported = await persistImportPeople(props.personType, importableRows.value.map(row => ({ id: row.id, prefix: row.prefix, firstName: row.firstName, lastName: row.lastName, ...(row.phone ? { phone: row.phone } : {}), ...(row.email ? { email: row.email } : {}), ...(props.personType === 'student' ? { cohortYear: Number(selectedCohortYear.value) } : {}), ...(row.cycle ? { cycle: row.cycle } : {}), ...(row.section ? { section: row.section } : {}) })))
...
<template>
  <div>
    <template v-if="stage === 'upload'">
      ...
      <UiSelect v-if="props.personType === 'student'" v-model="selectedCohortYear" class="mt-5 w-full sm:w-40" :options="academicYearOptions" label="ปีการศึกษา" />
    ...
    <template v-else-if="stage === 'preview'">
      ...
      <UiAlert v-if="props.personType === 'student'" class="mt-4" tone="info" title="ปีการศึกษาที่เลือก">นักศึกษาชุดนี้จะถูกบันทึกในปีการศึกษา {{ selectedCohortYear }}</UiAlert>
    ...
    <template v-else>
      ...
      <UiAlert v-if="props.personType === 'student'" class="mt-4" tone="info" title="ปีการศึกษาที่นำเข้า">บันทึกข้อมูลนักศึกษาในปีการศึกษา {{ selectedCohortYear }}</UiAlert>
